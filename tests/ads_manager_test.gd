...
# Headless test runner for AdsManager (AdMob iOS) fixes F1-F4.
...
func _test_normal_completed_flow_still_earns() -> void:
	...
	m._kick_off_flow(_make_label())
	m._on_ad_loaded(null)
	_check(m.is_flow_active(), "F2 first ad keeps flow active")
	m._on_ad_loaded(null)
